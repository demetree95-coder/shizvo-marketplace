...
async function run() {
  ...
  const adminId = uid();
  db.prepare('INSERT OR IGNORE INTO User (id, email, password, fullName, phone, role) VALUES (?, ?, ?, ?, ?, ?)')
    .run(adminId, 'admin@markaz.ge', password, 'ადმინი', '599111111', 'ADMIN');
  ...
  for (const v of vendors) {
    const id = uid();
    db.prepare('INSERT OR IGNORE INTO User (id, email, password, fullName, phone, role) VALUES (?, ?, ?, ?, ?, ?)')
      .run(id, v.email, password, v.name, v.phone, 'VENDOR');
    vendorIds.push(id);
  }

  const userId = uid();
  db.prepare('INSERT OR IGNORE INTO User (id, email, password, fullName, phone, role) VALUES (?, ?, ?, ?, ?, ?)')
    .run(userId, 'user@markaz.ge', password, 'მომხმარებელი', '599999999', 'USER');
  ...
  const catIds = [];
  const upsertCat = db.prepare('INSERT OR IGNORE INTO Category (id, name, slug, icon, "order") VALUES (?, ?, ?, ?, ?)');
  for (const c of cats) {
    const id = uid();
    upsertCat.run(id, c.name, c.slug, c.icon, c.order);
    catIds.push(id);
  }
  ...
  const shopIds = [];
  const upsertShop = db.prepare(`INSERT OR IGNORE INTO Shop (id, name, slug, description, isFeatured, userId, subscriptionPlan, subscriptionEnd, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  for (const s of shopData) {
    const id = uid();
    upsertShop.run(id, s.name, s.slug, s.desc, s.featured, s.userId, 'PREMIUM', yearEnd, now, now);
    shopIds.push(id);
  }
  ...
  const products = [
    ...
    { shop: 0, cat: 0, name: 'Samsung Galaxy S24', slug: 'samsung-galaxy-s24', price: 2799, discount: null, stock: 20, rating: 4.6, reviews: 35, sold: 95 },
    { shop: 0, cat: 0, name: 'AirPods Pro 2', slug: 'airpods-pro-2', price: 799, discount: 649, stock: 30, rating: 4.7, reviews: 56, sold: 210 },
    { shop: 0, cat: 0, name: 'iPad Air', slug: 'ipad-air', price: 2499, discount: null, stock: 12, rating: 4.5, reviews: 19, sold: 45 },
    { shop: 1, cat: 1, name: 'ქართული სვიტერი', slug: 'qartuli-sviteri', price: 129, discount: null, stock: 50, rating: 4.4, reviews: 15, sold: 89 },
    { shop: 1, cat: 1, name: 'კაბა - ელეგანტური', slug: 'kaba-eleganuri', price: 199, discount: 149, stock: 25, rating: 4.6, reviews: 22, sold: 156 },
    { shop: 1, cat: 2, name: 'ხელის ჩანთა', slug: 'xelis-chanta', price: 89, discount: null, stock: 40, rating: 4.3, reviews: 8, sold: 34 },
    { shop: 1, cat: 1, name: 'ჯინსის ქურთუკი', slug: 'jinsis-qurtuki', price: 159, discount: 119, stock: 18, rating: 4.7, reviews: 31, sold: 203 },
    { shop: 2, cat: 4, name: 'სავარჯიშო ფეხსაცმელი', slug: 'savajisho-pexsacmeli', price: 249, discount: null, stock: 35, rating: 4.5, reviews: 27, sold: 178 },
    { shop: 2, cat: 4, name: 'იოგას ხალიჩა', slug: 'yogasa-xalicha', price: 59, discount: null, stock: 60, rating: 4.2, reviews: 12, sold: 67 },
    { shop: 2, cat: 4, name: 'სპორტული ბოთლი', slug: 'sportuli-botli', price: 35, discount: null, stock: 100, rating: 4.1, reviews: 6, sold: 45 },
  ];

  const insertProduct = db.prepare(`INSERT INTO Product (id, name, slug, description, price, discountPrice, stock, images, features, deliveryInfo, sku, isActive, isFeatured, rating, reviewCount, soldCount, shopId, categoryId, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?)`);
  ...
  for (const p of products) {
    ...
    insertProduct.run(id, p.name, p.slug, desc, p.price, p.discount, p.stock, images, features, 'მიწოდება მთელი საქართველოს მასშტაბით 2-3 დღეში', sku, Math.random() > 0.5 ? 1 : 0, p.rating, p.reviews, p.sold, shopIds[p.shop], catIds[p.cat], now, now);
  }

  console.log('✅ Database seeded successfully!');
  console.log('👤 Admin: admin@markaz.ge / password123');
