...
export default function ShopDetailPage() {
  ...
  if (!shop) return <div className="min-h-screen pt-24 flex items-center justify-center"><p className="text-gray-500">{ge.common.noData}</p></div>;
    ...
    <div className="min-h-screen pt-20 md:pt-24 bg-gray-50">
      <div className="container-custom py-8">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                ...
                <span className="text-sm text-gray-500">{shop._count?.products || 0} {ge.shop.products}</span>
              </div>
              {shop.description && <p className="text-sm text-gray-500">{shop.description}</p>}
            </div>
            {user && (
              <button onClick={startChat} className="btn-secondary shrink-0"><HiOutlineChatBubbleLeftRight className="w-4 h-4" /> {ge.chat.title}</button>
            )}
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <h2 className="text-xl font-bold mb-4">{ge.shop.products}</h2>
          {products.length === 0 ? (
            <p className="text-gray-400 text-center py-12">{ge.shop.noProducts}</p>
          ) : (
